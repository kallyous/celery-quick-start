...
if __name__=='__main__':
    ...
    output_dir = os.path.join(project_location, 'data')
    ...
    # Celery Pure - Worker
    config_str_worker = buildProgramConfig('Celery Pure - Worker',
                                    'celerypure-worker',
                                    os.path.join(project_location, 'celery_worker.sh'),
                                    project_location,
                                    user,
                                    1,
                                    os.path.join(output_dir, 'celerypure-worker.log'),
                                    os.path.join(output_dir, 'celerypure-worker.log'),
                                    autostart=False,
                                    priority=998)

    # Celery Pure - Beat
    config_str_beat = buildProgramConfig('Celery Pure - Beat',
                                    'celerypure-beat',
                                    os.path.join(project_location, 'celery_beat.sh'),
                                    project_location,
                                    user,
                                    1,
                                    os.path.join(output_dir, 'celerypure-beat.log'),
                                    os.path.join(output_dir, 'celerypure-beat.log'),
                                    autostart=False,
                                    priority=999)

    # Escreve configuração do worker.
    filepath = os.path.join(supervisor_d_dir, 'celerypure-worker.ini')
    with open(filepath, 'w') as file:
        file.write(config_str_worker)

    # Escreve configuração do beat.
    filepath = os.path.join(supervisor_d_dir, 'celerypure-beat.ini')
    with open(filepath, 'w') as file:
